...
%
\node[mtxo, align=center, rotate=90](reallbl) at (-1.75,0){real label};
\node[mtxo, align=center](predlbl) at (0,1.75){predicted label};
%
%
\node[mtxo, align=left](realfalse) at (2.25,+0.75){$R_0 = TN+FP$};
\node[mtxo, align=left](realtrue) at (2.25,-0.75){$R_1 = FN+TP$};
%
\node[mtxo, align=center](predfalse) at (-0.75,-1.75){$P_0 =$\\$TN+FN$};
\node[mtxo, align=center](predtrue) at (+0.75,-1.75){$P_1 =$\\$FP+TP$};
%
\end{tikzpicture}
\end{center}

\begin{align*}
\text{accuracy}=\frac{TP+TN}{R_0+R_1}=\frac{TP+TN}{P_0+P_1}
\qquad
\text{recall}=\frac{TP}{R_1}
\qquad
\text{precision}=\frac{TP}{P_1}
\end{align*}
%
\end{frame}
...
\begin{frame}[t]{Recall / Precision}
%
$\cdot$ extrem cases
\begin{align*}
&\text{recall}=\frac{TP}{R_1}=\frac{TP}{TP+FN}
&\text{precision}=\frac{TP}{P_1}=\frac{TP}{TP+FP}\\
&\text{recall}\approx 0:    FN \gg TP,\,\,\, TP \approx 0
&\text{precision}\approx 0: FP \gg TP,\,\,\, TP \approx 0\\
&\text{recall}\approx 1:    TP \gg FN,\,\,\, FN \approx 0
&\text{precision}\approx 1: TP \gg FP,\,\,\, FP \approx 0\\
\end{align*}
%
$\cdot$ $\text{recall}\approx 1$ and $\text{precision}\approx 0$: few FN but many FP (e.g. overestimation of infections)

$\cdot$ $\text{recall}\approx 0$ and $\text{precision}\approx 1$: few FP but many FN (e.g. we think we are safe, but are not)

$\cdot$ a potentially meaningful(?!) combination
$$
\left(\frac{\frac{1}{\text{recall}} + \frac{1}{\text{precision}}}{2}\right)^{-1}
=
