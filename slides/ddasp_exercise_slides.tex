...
$$
%
optimization problem in least squares sense: $\min_{\text{wrt }\bm{\theta}} ||\bm{e}||_2^2 = \min_{\text{wrt }\bm{\theta}} ||\bm{y} - \bm{X} \bm{\theta}||_2^2$
%

recall that $||\bm{y} - \bm{X} \bm{\theta}||_2^2 = (\bm{y} - \bm{X} \bm{\theta})^H (\bm{y} - \bm{X} \bm{\theta})$
\begin{align*}
||\bm{y} - \bm{X} \bm{\theta}||_2  &= \sqrt{(-3 - 3\theta_1)^2 + (4-8\theta_2)^2 + (0-2)^2}\\
J(\theta_1, \theta_2) = ||\bm{y} - \bm{X} \bm{\theta}||_2^2 &= (-3 - 3\theta_1)^2 + (4-8\theta_2)^2 + (0-2)^2
\end{align*}
%
$$
...
\begin{frame}[t]{Left Inverse in SVD Mindset I}
...
\begin{frame}[t]{Left Inverse in SVD Mindset II}
...
\begin{frame}[t]{Left Inverse in SVD Mindset III}
...
\begin{frame}[t]{Left Inverse in SVD Mindset IV}
...
\begin{frame}[t]{Left Inverse in SVD Mindset V}
...
\begin{frame}[t]{Left Inverse in SVD Mindset VI}
...
\begin{frame}{Another Treatment for Left Inverse Via SVD}
