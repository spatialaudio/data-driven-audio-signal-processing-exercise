...
\date[Winter Term 2026/27]{%\raisebox{0mm}{\includegraphics[width=4.6cm]{logo.png}}\\
  Tutorial -- Winter Term 2026/27\\
...
\begin{frame}{Literature}
  ...
  \begin{itemize}
    ...
    \item J.A. Fessler, R.R. Nadakuditi (2024): "Linear Algebra for Data Science, Machine Learning, and Signal Processing", Cambridge University Press, 1st ed. \href{https://ebookcentral.proquest.com/lib/ubrostock-ebooks/detail.action?docID=31691281}{free online ebook}
    \item J. Fox (2008/2016): "Applied Regression Analysis and Generalized Linear Models", Sage, 2nd/3rd ed.
    \end{itemize}
\end{frame}
...
\begin{frame}{Resources}
  ...
  \begin{itemize}
    ...
    \item courses by Andrew Ng at \url{https://www.deeplearning.ai/} and/or at \url{https://www.coursera.org/}
  \end{itemize}
\end{frame}
...
\begin{frame}{Textbooks on Statistics}
  ...
  \item A. Agresti, M. Kateri, R. Grove, A. Mira, Foundations of Bayesian Statistics for Data Scientists, CRC, 2026
  \end{itemize}
\end{frame}
...
purely real eigenvalues $\textcolor{C0}{\bm{\Lambda}\in\mathbb{R}}$

$\bm{Q}\in\mathbb{R}$ if $\bm{A}\in\mathbb{R}$, \qquad $\bm{Q}\in\mathbb{C}$ if $\bm{A}\in\mathbb{C}$
...
for a \textcolor{C3}{\underline{normal}} matrix $\bm{A}$ (i.e. it holds $\bm{A}^H \bm{A} = \bm{A} \bm{A}^H$, note: every Hermitian is normal)

and a dedicated \textcolor{C3}{\underline{unitary}} matrix $\bm{Q}$ (i.e. it holds $\bm{Q} \bm{Q}^H = \bm{I}$, $\bm{Q}^H \bm{Q} = \bm{I}$)

special case of diagonalization, known as \textcolor{C3}{\underline{spectral theorem}}:
$$\bm{A} = \bm{Q} \bm{\Lambda} \bm{Q}^{-1} = \bm{Q} \bm{\Lambda} \bm{Q}^{H}$$

with $M$ independent, \underline{orthonormal} eigenvectors as columns in $\textcolor{C3}{\bm{Q} \in\mathbb{R} \,\,\,\text{or}\,\,\, \bm{Q} \in\mathbb{C}}$

with the corresponding eigenvalues in $\textcolor{C3}{\bm{\Lambda} \in\mathbb{R} \,\,\,\text{or}\,\,\, \bm{\Lambda} \in\mathbb{C}}$
...
\begin{frame}{SVD and the 4 Matrix Subspaces}
...
\begin{frame}{SVD Fundamentals Recap}
%
...
\begin{align*}
\lVert \bm{y} - \bm{X} \bm{\theta}\rVert_2  &= \sqrt{(-3 - 3\theta_1)^2 + (4-8\theta_2)^2 + (2-0)^2}\\
J(\theta_1, \theta_2) = \lVert\bm{y} - \bm{X} \bm{\theta}\rVert_2^2 &= (-3 - 3\theta_1)^2 + (4-8\theta_2)^2 + (2-0)^2
\end{align*}
%
$$
...
\begin{frame}[t]{Left Inverse and 4 Subspaces of a Matrix}
\hspace{-0.5cm}
\textcolor{C2}{row space} $\perp$ \textcolor{C1}{null space}
\hspace{0.5cm}
\textcolor{C0}{column space} $\perp$ \textcolor{C4}{left null space}
...
\begin{flushleft}
\includegraphics[width=0.5\textwidth]{pseudo_inverse_four_subspaces.pdf}
\end{flushleft}
...
$\bm{X}_r = \bm{X}_c^\mathrm{T} = \bm{V} \cdot \bm{\Sigma}^\mathrm{T} \bm{U}^\mathrm{T} = \bm{L} \cdot \bm{F}_r$

$\bm{F}_r = \bm{L}^\mathrm{T} \bm{X}_r = \bm{V}^\mathrm{T} \bm{X}_r = \bm{\Sigma}^\mathrm{T} \bm{U}^\mathrm{T} = ( \bm{U} \bm{\Sigma})^\mathrm{T}$
...
\subsection{Exercise TBA}
\begin{frame}{Ex TBA: Gradient Descent}
...
$\cdot$ Output layer exhibits $K$ perceptrons with indexing $1 \leq i \leq K$
...
$\cdot$ Activation functions $\sigma(\cdot)_{1,2}$ for the two perceptrons: \underline{sigmoid} vs. complementary sigmoid
...
\begin{axis}[
    ...
    ylabel={activation function output $\sigma(z)$},
    xlabel={activation function input $z$},
    ]
...
$\cdot$ Well known as \underline{binary logistic regression} for a model with a single output layer only, but no other (non-linear, hidden) layers $\rightarrow$ cf. textbooks on generalized linear models (GLMs)
...
$\cdot$ Output layer exhibits $K$ perceptrons with indexing $1 \leq i \leq K$ for $K$ mutually exclusive classes
...
\text{, this normalisation couples the perceptrons}
$$
%which couples the perceptrons in the output layer

$\cdot$ Hence, Jacobian matrix needed to set up all partial derivatives
\begin{align*}
...
$\cdot$ Well known as \underline{multinomial logistic regression} for a model with a single output layer only, but no other (non-linear, hidden) layers $\rightarrow$ cf. textbooks on generalized linear models (GLMs)
...
$\cdot$ \textcolor{C0}{Regression} with least squares for predicted data sample $\hat{y}_i = \sigma_\text{\textcolor{C0}{linear}}(z_i)$; \quad $\bm{y}\in\mathbb{R}, \hat{\bm{y}}\in\mathbb{R}$

$$\mathcal{L}(\bm{y}, \hat{\bm{y}}) = \sum\limits_{i=1}^{K} (y_i-\hat{y}_i)^2$$

$\cdot$ \textcolor{C0}{Binary classification} for predicted data sample $\hat{y} = \sigma_\text{\textcolor{C0}{sigmoid}}(z)$; \quad $y \in \{0,1\}, 0 \leq \hat{y}\in\mathbb{R} \leq 1$
...
\begin{frame}{Ex10: Non-Linear Model Introduction}
Objectives
\begin{itemize}
...
\begin{frame}[t]{Recap Logical Mappings...Simple Classification Models}
...
$$\bm{X} \bm{\theta} = \bm{y} \quad\rightarrow\quad \hat{\bm{\theta}} = (\bm{X}^\mathrm{T} \bm{X})^{-1} \bm{X}^\mathrm{T} \bm{y}
=
\frac{1}{3}\cdot
\begin{bmatrix}
1\\
...
\end{bmatrix}
$$
Prediction (again useless for XOR)
\begin{align*}
\bm{X} \hat{\bm{\theta}} =
\frac{1}{2}
\begin{bmatrix}
...
$\cdot$ weight matrix and bias vector to represent perceptrons \textcolor{C0}{1} and \textcolor{C3}{2}
$$
\bm{W}_\text{layer 1} =
\begin{bmatrix}
...
\begin{frame}{Ex11: Binary Classification}
Objectives
\begin{itemize}
\item binary classifier as very simple non-linear model using sigmoid activation function
...
$\cdot$ Loss function $\mathcal{L}$ for $y\in\{0,1\}$ and predicted output $0 \leq \textcolor{C0}{\hat{y}} \in\mathbb{R} \leq 1$ / empirical risk $\mathcal{R}$
...
&\operatorname*{argmin}_{\bm{w}, b} \frac{1}{N} \sum\limits_{n=1}^N \mathcal{L}_n(y, \textcolor{C0}{\hat{y}}) =
...
3. get gradient of $J$ and numerically evaluate all those required derivatives
$$\left[
...
Loss $\mathcal{L}$ / empirical risk $\mathcal{R} = J(\bm{w},b)$
$$\mathcal{L}_n(y, \textcolor{C0}{\hat{y}}) = -\left[y_n \log_\e(\textcolor{C0}{\hat{y}_n}) + (1-y_n) \log_\e(1-\textcolor{C0}{\hat{y}_n})\right]
\qquad\quad
...
$$
%
We need to analytically know these derivatives for numerical evaluation with actual states, cf. step 4 on previous slide
4, hence we need to manually write down the equations for
$$
...
Check for $w_1$
\begin{align*}
&z = \zeta(w_1) = \bm{w}^\mathrm{T}\bm{x} + b = w_1 x_1 + w_2 x_2 + \cdots + w_F x_F + b\\
...
% round(dLdb,prec)
\begin{align*}
&z_\text{act} = 3.25 \quad \hat{y}_\text{act} = \hat{y}(w_{:,\text{act}}, b_{\text{act}}) = 0.962673\\
&\frac{\partial \mathcal{L}}{\partial \textcolor{C0}{\hat{y}}}\bigg|_{\hat{y}(w_{:,\text{act}}, b_{\text{act}})} = 54.619454\quad
\frac{\partial \textcolor{C0}{\hat{y}}}{\partial z}\bigg|_{z(w_{:,\text{act}}, b_{\text{act}})}= 0.035933\\
&\frac{\partial \mathcal{L}}{\partial w_1}\bigg|_{w_{1,\text{act}}} = 1.962673\quad
\frac{\partial \mathcal{L}}{\partial w_2}\bigg|_{w_{2,\text{act}}} = 3.925346\\
&\frac{\partial \mathcal{L}}{\partial w_3}\bigg|_{w_{3,\text{act}}} = 5.888019\quad
\frac{\partial \mathcal{L}}{\partial b}\bigg|_{b_{\text{act}}} = 1.962673
\end{align*}
...
\begin{frame}{Ex12: Binary Classification with Hidden Layer Model / Multivariate Chain Rule / Metrics}
Objectives
\begin{itemize}
\item binary classification model with hidden layers means more complexity, more back prop effort
\item back prop pathes for a multi-label classification with sigmoid outputs
\item metrices for binary classification from the 2x2 confusion matrix
\end{itemize}
\end{frame}
...
\node[hl](l1p1) at (2,+1.25){$\sigma_{1}(\underbrace{\bm{w}_{11}^\mathrm{T} \bm{a}_0 + b_{11}}_{z_{11}})$};
\node[hl](l1p2) at (2,-1.25){$\sigma_{1}(\underbrace{\bm{w}_{21}^\mathrm{T} \bm{a}_0 + b_{21}}_{z_{21}})$};
\node[hl](l2p1) at (5,+1.25){$\sigma_{2}(\underbrace{\bm{w}_{12}^\mathrm{T} \bm{a}_{1} + b_{12}}_{z_{12}})$};
\node[hl](l2p2) at (5,-1.25){$\sigma_{2}(\underbrace{\bm{w}_{22}^\mathrm{T} \bm{a}_{1} + b_{22}}_{z_{22}})$};
\node[hl](lop1) at (8,0){$\sigma_{3}(\underbrace{{\bm{w}_{13}^\mathrm{T}\bm{a}_{2} + b_{13}}}_{z_{13}})=\hat{y}$};
\node[](y) at (9.75,0){};
%
\draw[->] (x0) -- (l1p1);
\draw[->] (x0) -- (l1p2);
...
%
\draw[->,C0, thick] (l1p1) -- (l2p1);
\draw[->,C2, thick] (l1p1) -- (l2p2);
\draw[->] (l1p2) -- (l2p1);
\draw[->] (l1p2) -- (l2p2);
%
\draw[->, C0, thick] (l2p1) -- (lop1);
\draw[->, C2, thick] (l2p2) -- (lop1);
%
...
$};
%
\node[](a3) at (9.5,2.5){$\bm{a}_3 = [a_{13}] = \hat{y}$};
\end{tikzpicture}
\end{center}
...
\frac{\partial z_{13}}{\partial a_{12}}}
\right]\cdot
...
\left[
\textcolor{C2}{
\frac{\partial z_{22}}{\partial a_{11}}\cdot
\frac{\partial a_{22}}{\partial z_{22}}\cdot
\frac{\partial z_{13}}{\partial a_{22}}
}
\right]\cdot
\frac{\partial a_{13}}{\partial z_{13}}\cdot
\frac{\partial \mathcal{L}}{\partial a_{13}}
...
\only<2>{
\begin{align*}
\text{back-prop with post-activation variables:}\,
\frac{\partial \mathcal{L}}{\partial w_{11,x_1}}
=
&\frac{\partial a_{11}}{\partial w_{11,x_1}}\cdot
\left[
\textcolor{C0}{
\frac{\partial a_{12}}{\partial a_{11}}\cdot
\frac{\partial a_{13}}{\partial a_{12}}
}
\right]\cdot
\frac{\partial \mathcal{L}}{\partial a_{13}}
\textcolor{C3}{+}\\
...
\left[
\textcolor{C2}{
\frac{\partial a_{22}}{\partial a_{11}}\cdot
\frac{\partial a_{13}}{\partial a_{22}}
}
\right]\cdot
\frac{\partial \mathcal{L}}{\partial a_{13}}
\end{align*}}
\only<3>{
\begin{align*}
\text{back-prop with pre-activation variables:}\,
\frac{\partial \mathcal{L}}{\partial w_{11,x_1}}
=
&x_1\cdot
\left[
...
\node[hl](l2p1) at (5,+1.25){$\sigma_{2}(\underbrace{\bm{w}_{12}^\mathrm{T} \bm{a}_{1} + b_{12}}_{z_{12}})$};
\node[hl](l2p2) at (5,-1.25){$\sigma_{2}(\underbrace{\bm{w}_{22}^\mathrm{T} \bm{a}_{1} + b_{22}}_{z_{22}})$};
\node[hl](lop1) at (8,+1.25){$\sigma_{3}(\underbrace{{\bm{w}_{13}^\mathrm{T}\bm{a}_{2} + b_{13}}}_{z_{13}})$};
\node[hl](lop2) at (8,-1.25){$\sigma_{3}(\underbrace{{\bm{w}_{23}^\mathrm{T}\bm{a}_{2} + b_{23}}}_{z_{23}})$};
\node[](yh1) at (9.5,+1.25){};
\node[](yh2) at (9.5,-1.25){};
%
\draw[->] (x0) -- (l1p1);
...
%
\draw[->] (l1p1) -- (l2p1)node[midway, above]{$a_{11}$};
\draw[->, C0, ultra thick] (l1p1) -- (l2p1);
\draw[->, C1, dashed, thick] (l1p1) -- (l2p1);
\draw[->, C2, ultra thick] (l1p1) -- (l2p2);
\draw[->, C3, dashed, thick] (l1p1) -- (l2p2);
\draw[->] (l1p2) -- (l2p1);
\draw[->] (l1p2) -- (l2p2)node[midway, below]{$a_{21}$};
%
\draw[->] (l2p1) -- (lop1)node[midway, above]{$a_{12}$};
\draw[->, C0, thick] (l2p1) -- (lop1);
\draw[->, C1, thick] (l2p1) -- (lop2);
\draw[->, C2, thick] (l2p2) -- (lop1);
\draw[->] (l2p2) -- (lop2)node[midway, below]{$a_{22}$};
\draw[->, C3] (l2p2) -- (lop2);
%
\draw[->] (lop1) -- (yh1)node[right]{$a_{13}$};
\draw[->] (lop2) -- (yh2)node[right]{$a_{23}$};
%
\node[](a0) at (0.25,2.5){$
\bm{x} = \bm{a}_0 =
\begin{bmatrix}
...
\begin{frame}{Ex13: Multi-Class Classification with Softmax Output Layer}
Objectives
\begin{itemize}
...
%
\node[iol](y1) at (9,+2.67){$\hat{y}_1$};
\node[iol](y2) at (9,+0){$\hat{y}_2$};
\node[iol](y3) at (9,-2.62){$\hat{y}_3$};
%
\draw[->] (x1) -- (dl);
\draw[->] (x2) -- (dl);
\draw[->] (x3) -- (dl);
...
\draw[->] (lop3) -- (y3);
%
\node at (11.4,+3.46){softmax activation:};
\node at (11.85,+2.46){$\hat{y}_i = \sigma(z_{i\text{o}})=\frac{\mathrm{exp}(z_{i\text{o}})}{\sum\limits_{i'=1}^{K} \mathrm{exp}(z_{i'\text{o}})}$};
\node at (11.85,+1.46){$\sum\limits_{i=1}^{K} \hat{y}_i = 1$};
\end{tikzpicture}
\end{flushleft}
%
\end{frame}
...
\begin{frame}
for example $K=2$ classes:

pre-activation softmax output layer $$z_1, z_2$$

post-activation softmax output layer $$\hat{y}_1, \hat{y}_2$$

ground truth variables (one-hot encoded, $\in\{0,1\}$) $$y_1, y_2,\quad y_1+y_2=1$$

loss function CCE
$$\mathcal{L} = -\left(y_1 \log_\e(\hat{y}_1) + y_2 \log_\e(\hat{y}_2)\right)
\rightarrow\qquad
\frac{\partial \mathcal{L}}{\hat{y}_1} = -\frac{y_1}{\hat{y}_1}
\qquad
\frac{\partial \mathcal{L}}{\hat{y}_2} = -\frac{y_2}{\hat{y}_2}
$$
...
$$
\begin{bmatrix}
\frac{\partial \hat{y}_1}{\partial z_1} & \frac{\partial \hat{y}_1}{\partial z_2}\\
\frac{\partial \hat{y}_2}{\partial z_1} & \frac{\partial \hat{y}_2}{\partial z_2}
\end{bmatrix}
=
\begin{bmatrix}
\hat{y}_1 (1-\hat{y}_1) & -\hat{y}_1 \hat{y}_2\\
-\hat{y}_2 \hat{y}_1 & \hat{y}_2 (1-\hat{y}_2)
\end{bmatrix}
$$
\end{frame}
...
\end{bmatrix}
$$
pre-activation handling requires
$$
\frac{\partial \mathcal{L}}{\partial z_{\textcolor{C1}{1}}} = \sum\limits_{j=1}^{K=2} \frac{\partial \mathcal{L}}{\partial \hat{y}_j} \frac{\partial \hat{y}_j}{\partial z_{\textcolor{C1}{1}}}
\qquad
\frac{\partial \mathcal{L}}{\partial z_{\textcolor{C2}{2}}} = \sum\limits_{j=1}^{K=2} \frac{\partial \mathcal{L}}{\partial \hat{y}_j} \frac{\partial \hat{y}_j}{\partial z_{\textcolor{C2}{2}}}
$$
in detail for $z_{\textcolor{C2}{1}}$:
$$
\frac{\partial \mathcal{L}}{\partial z_{\textcolor{C1}{1}}}
=
\frac{\partial \mathcal{L}}{\partial \hat{y}_1} \frac{\partial \hat{y}_1}{\partial z_{\textcolor{C1}{1}}}+\frac{\partial \mathcal{L}}{\partial \hat{y}_2} \frac{\partial \hat{y}_2}{\partial z_{\textcolor{C1}{1}}}
$$
insert from above:
$$
\frac{\partial \mathcal{L}}{\partial z_{\textcolor{C1}{1}}}
...
(-\frac{y_2}{\hat{y}_2})\cdot(-\hat{y}_2 \hat{y}_1)
=
-y_1(1-\hat{y}_1) + y_2 \hat{y}_2 = y_1 \hat{y}_1 + y_2 \hat{y}_1 - y_1
$$
\end{frame}
...
\begin{frame}[t]{Confusion Matrix for Multi-Class Classification}
\begin{center}
\begin{tikzpicture}[scale=1]
...
\node[mtxo, align=center, rotate=90](reallbl) at (-2.1,0){\textcolor{C3}{real} class};
%
...
\def\N{5}
\def\rank{0.999999}
\drawmatrix[fill=none, height=\F, width=\N]{X^T}_\mathtt{F \times N}
\drawmatrix[fill=none, height=\K, width=\N]{Y^T}_\mathtt{K \times N}
$
\end{center}
...
%
% \begin{frame}[t]{Manual Calculus SVD of Simple 2x2 Matrix Example: Problem Definition and First Expectations}
...
%
% The $\bm{U}$ space has $M$ vectors in $\mathbb{R}^{M=2}$, the $\bm{V}$ space has $N$ vectors in $\mathbb{R}^{N=2}$ (these are bases).
%
% \end{frame}
%
%
...
% is 2D, so no more than two independent lines are possible in these spaces. In the SVD bases $\bm{U}$ and $\bm{V}$, these lines are orthogonal.
%
...
% 15 & 45
% \end{bmatrix}$$
% has dependent rows and columns, hence, still rank $R=1$
%
% \end{frame}
%
%
...
% $\bm{X}^\mathrm{T} \bm{X}$.
% We need to calculate this eigen-problem, so we take the usual steps
...
%
% only one non-zero eigenvalue directly corresponds to rank $R=1$
%
% \end{frame}
%
%
...
% (\bm{X}^\mathrm{T} \bm{X}) \bm{v}_2 = \lambda_2 \bm{v}_2$$
% Rearranging
% $$
% (\bm{X}^\mathrm{T} \bm{X} - \lambda_1 \bm{I} )\bm{v}_1 = \bm{0}\quad
% (\bm{X}^\mathrm{T} \bm{X} - \lambda_2 \bm{I} )\bm{v}_2 = \bm{0}
% $$
...
% to find the corresponding $r$-th unit-length vector $\bm{u}_r$ in the column space, simply by
...
%
% So, the full right singular vector matrix and the singular value matrix  are set up as
%
% $$
% \bm{V} =
% \begin{bmatrix}
...
\item the text is licensed under \href{https://creativecommons.org/licenses/by/4.0/}{Creative Commons Attribution 4.0 International}
\item the code snippets in the $*$.py / $*$.ipynb examples are licensed under the \href{https://opensource.org/licenses/MIT}{MIT license}
