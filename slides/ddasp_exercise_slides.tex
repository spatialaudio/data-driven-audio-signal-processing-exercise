...
pseudo-inverse maps columns space to row space

we can derive 4 matrices from the pseudo-inverse that project into the 4 subspaces
...
Such a model has $N$ model parameters $\bm{\theta} = [\theta_1, \theta_2 \dots \theta_N]^T$ and one hyper parameter $\lambda$ to be learned from the data $\bm{y}$ and the feature matrix $\bm{X}$.
